...
- k small â†’ overfitting
- k large â†’ underfitting
...
- Likelihood: $p(D | \lambda) = \prod_i^N \frac{\lambda^{x_i} e^{-\lambda}}{x_i!}$
...
- Batch size $\downarrow$ â†’ variance $\uparrow$
- Batch size $\uparrow$ â†’ computation time $\uparrow$
...
- If $0 < \alpha_i < C$: $\xi_i = 0$ (point exactly on margin)
- If $\alpha_i = C$: $\xi_i > 0$ (point violates margin)
- Larger $C$ â†’ less tolerance for points inside margin
...
**Kernel Matrix Properties**: Must be symmetric positive semi-definite
...
**RMSE**: $RMSE = \sqrt{\frac{1}{|S|} \sum (r_{ui} - \hat{r}_{ui})^2}$

**SSE**: $SSE = \sum (r_{ui} - [U \Sigma V^T]_{ui})^2$
...
```
      Scatter Plot (High Dim)            Adjacency Matrix (Graph)
      ^
      |   x                               x x x o o o o o
      | x   x                             x x o . . . . .
      |       o                           x o x . . . . .
...
2. **E-step (Expectation)**:
    ...
    - _Note_: E-step evaluates the posterior $p(z|x, \theta^t)$.
...
- **Method**: Merge clusters recursively.
...
<head>
    ...
    <title>Ultimate ML Cheat Sheet (Full Content)</title>
    <link href="https://fonts.googleapis.com/css2?family=Caveat:wght@400;600;700&display=swap" rel="stylesheet">
    <script>
        window.MathJax = {
            tex: { inlineMath: [['$', '$'], ['\\(', '\\)']], displayMath: [['', '']] },
            chtml: { scale: 0.75, displayAlign: 'left' }, // Reduced scale further
            startup: { pageReady: () => MathJax.startup.defaultPageReady().then(() => humanizeMath()) }
        };
    </script>
    <script id="MathJax-script" async src="https://cdn.jsdelivr.net/npm/mathjax@3/es5/tex-mml-chtml.js"></script>
    <style>
        :root {
            --hl-y: rgba(255, 230, 0, 0.4); --hl-c: rgba(0, 240, 255, 0.3);
            --hl-g: rgba(50, 255, 50, 0.25); --hl-p: rgba(255, 0, 100, 0.2);
            --text: #1a1a1a;
        }
        * { box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
        body { 
            font-family: 'Caveat', cursive; margin: 0; padding: 0; background: #fff; color: var(--text);
            font-size: 7.5pt; line-height: 1.1; /* Very compact */
        }
        mjx-container { font-family: inherit; color: #000 !important; margin: 0 !important; }
        
        .page-container {
            width: 210mm;
            /* Allow height to flow naturally for column balancing, or fixed pages? */
            /* User wants 2 pages strict. Let's try column-count on body or massive div */
            column-count: 3;
            column-gap: 2mm;
            column-fill: auto;
            padding: 3mm;
            height: 594mm; /* 2 * 297mm - Exact 2 pages height */
            /* Actually, browsers handle height weirdly with columns across pages. 
               Better to just let it flow and hope it fits in 2 pages physically. */
        }

        /* Let's try explicit page breaks if needed, or just one massive flow */
        @media print {
            body { margin: 0; }
            .page-break { page-break-after: always; }
        }

        h1 { font-size: 12pt; text-align: center; border-bottom: 2px solid #000; margin: 2px 0 4px 0; font-weight: 700; column-span: all; }
        h2 { font-size: 9.5pt; border-bottom: 1px solid #555; margin: 3px 0 2px 0; font-weight: 700; padding-left: 2px; page-break-after: avoid; }
        h3 { font-size: 8pt; margin: 2px 0 1px 0; font-weight: 700; text-decoration: underline; }
        p, li { margin: 0 0 1px 0; }
        ul { margin: 0 0 0 12px; padding: 0; }
        
        .box {
            break-inside: avoid; margin-bottom: 4px; padding-bottom: 2px;
            border-bottom: 1px dashed #ccc; position: relative;
        }
        
        /* Classes */
        .hl-y { background: var(--hl-y); padding: 0 2px; border-radius: 3px; }
        .hl-c { background: var(--hl-c); padding: 0 2px; border-radius: 3px; }
        .hl-g { background: var(--hl-g); padding: 0 2px; border-radius: 3px; }
        .hl-p { background: var(--hl-p); padding: 0 2px; border-radius: 3px; }
        
        .b { font-weight: 700; }
        
        table { border-collapse: collapse; width: 100%; font-size: 0.9em; margin: 2px 0; }
        td, th { border: 1px solid #444; padding: 0 2px; text-align: center; }
        
        pre {
            font-family: monospace; font-size: 0.7em; white-space: pre-wrap;
            margin: 1px 0; background: #f0f0f0; padding: 2px; border-radius: 2px;
        }
    </style>
</head>
<body>
...
<script>
    ...
    function humanizeElements() {
        document.querySelectorAll('.box').forEach(box => {
            box.style.transform = otate(deg) translate(px, px);
        });
        document.querySelectorAll('h2').forEach(h => h.style.transform = otate(deg));
    }
    function humanizeMath() {
        document.querySelectorAll('mjx-container').forEach(mjx => mjx.style.transform = 	ranslateY(px));
    }
    humanizeElements();
</script>
</body>
...
def parse_full_markdown(text):
    ...
    # Helper to flush current box
    def flush_box():
        nonlocal current_box
        if current_box:
            output_html = f'<div class="box">\n'
            output_html += f'<h2 class="{current_box["color"]}">{current_box["title"]}</h2>\n'
            output_html += "\n".join(current_box["content"])
            output_html += f'</div>\n'
            return output_html
        return ""
    ...
    i = 0
    while i < len(lines):
        line = lines[i].rstrip() # Keep indentation? MD uses indentation.
        
        # --- Code Blocks ---
        if line.strip().startswith('`'):
            if in_code_block:
                # End block
                # Convert code content to pre
                if current_box:
                    current_box['content'].append(f'<pre>{code_content}</pre>')
                else:
                    # Orphan code?
                    pass
                in_code_block = False
        ...
        # --- Headers ---
        if stripped.startswith('# '): # Level 1
            # Close previous box
            output_html += flush_box()
            current_box = None
            # Treat Level 1 as a big separator or just ignore and let columns flow?
            # User wants cheat sheet. # usually indicates topic.
            # Let's make it a full width header? Or just a box header?
            # Let's make it a bold H1 in the column flow.
            title = stripped[2:].strip()
            output_html += f'<h1>{title}</h1>'
            
        elif stripped.startswith('## '): # Level 2 - Box Start
            output_html += flush_box()
        ...
        elif stripped.startswith('### '): # Level 3
            if current_box:
                current_box['content'].append(f'<h3>{stripped[4:].strip()}</h3>')
            else:
                # Fallback if content starts with ###
                output_html += f'<h3>{stripped[4:].strip()}</h3>'
        ...
        # --- Tables ---
            ...
            # Identify header (row 1 is dashes)
            header = []
            body = rows
            if len(rows) > 1 and set(rows[1][0]) <= set('- :'):
                header = rows[0]
                body = rows[2:]
            
            tbl = '<table>'
            if header:
                tbl += '<tr>' + ''.join([f'<th>{c}</th>' for c in header]) + '</tr>'
            for r in body:
                tbl += '<tr>' + ''.join([f'<td>{c}</td>' for c in r]) + '</tr>'
            tbl += '</table>'
            
            if current_box:
                current_box['content'].append(tbl)
            else:
                output_html += tbl
        ...
        # --- Content ---
            ...
            # Handle math blocks  ...  separately? MathJax handles them if passed through.
            # We just need to protect them from markdown parsing if we were using a lib, but here we are manual.
            
            content_line = stripped
            
            # Bold
            content_line = re.sub(r'\*\*(.*?)\*\*', r'<span class="b">\1</span>', content_line)
            
            # List
            if content_line.startswith('- '):
                 content_line = '&bull; ' + content_line[2:]
            
            # Wrap
            if current_box:
                current_box['content'].append(f'<p>{content_line}</p>')
            else:
                # Content outside box? e.g. Intro text
                # Append to output directly
                output_html += f'<p>{content_line}</p>'
...
body_content = parse_full_markdown(md_text)

full_html = html_head + body_content + html_footer

with open("ChSt_Gemini.html", "w", encoding="utf-8") as f:
    f.write(full_html)
